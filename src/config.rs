...
impl Config {
    pub fn new(client_id: String, client_secret: String, instance: String, token: String) -> Self {
        Config {
            client_id,
            client_secret,
            instance,
    ...
    pub fn create(config_path: Option<PathBuf>, mut config: Config) -> eyre::Result<Config> {
        let dirs = crate::dirs::new()?;
        let config_path = config_path.ok_or(()).or_else(|()| {
            dirs.place_config_file("config.json")
                .wrap_err("unable to create path to config file")
